...
## User-facing architecture documents

The maintained entry point is the parent `DeepXDE_adaptive_Final` folder. Its separate
TOML documents select the network, optimization, collocation, sensor, and
online-update settings without copying this workflow. Use that folder for new
architecture comparisons; `parameters.py` remains the central definition of
the shared physics and analytical problem.
...
By default this compares only `balanced` and `low_latency` against the current
analytical event reference. Use `--all-experiments` only when the historical
ablation configurations need to be regenerated.
