...
def plot_streaming_sensors(cfg, data, output_dir: Path, reference_slice=None):
    ...
    fig.colorbar(
        image,
        ax=ax,
        label="Dimensionless reference temperature",
    )
    ...
    segment_labels = tuple(
        f"segment {index + 1}: tau={start:g}-{stop:g}"
    ...
    ax.axhline(
        ...
        linestyle="--",
        linewidth=1.4,
        label=f"boundary change at tau={data.switch_tau:g}",
    )
    ax.set(
        xlabel="Dimensionless wall coordinate X at Y=0.5",
        ylabel=r"Dimensionless time, $\tau$",
        title="Measurements assimilated by the low-latency adaptive PINN",
        xlim=(0.0, 1.0),
    ...
    _caption(
        ...
        "training batches. The model updates after every time instance (n=1) using 25 sensors "
        "and retains only the four most recent time instances in its data-loss window. The "
        "background is the analytical centerline temperature on a linear color scale.",
    )
...
def plot_error_fields(cfg, data, baseline_prediction, adaptive, output_dir: Path):
    ...
    axes[0].set_title("Offline PINN: prediction - reference")
    axes[1].set_title("Adaptive PINN after update: prediction - reference")
    axes[2].set_title("Absolute-error reduction from adaptation")
    ...
    fig.colorbar(
        ...
        fraction=0.035,
        pad=0.025,
        label="Signed dimensionless temperature error",
    )
    fig.colorbar(
        third,
        ax=axes[2],
...
def main():
    ...
    combined = next(
        ...
        if experiment.name == "low_latency"
    )
