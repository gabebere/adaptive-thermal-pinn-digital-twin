...
def validate_against_literature(cfg: WorkflowConfig, output_dir: Path) -> dict:
    ...
    for index, (table, set_name) in enumerate(TABLE_TO_SET.items()):
        ...
        ax.set(title=f"{table}: RMSE={published_rmse:.3g}", xlabel="tau")
        ax.grid(alpha=0.25)
    axes[0].set_ylabel("theta(0.5, 0.5, tau)")
    axes[0].legend(fontsize=8)
    fig.suptitle(
        "Paper midpoint check: Table 1 is consistent; Tables 2-3 conflict with the stated equations"
    )
    fig.tight_layout()
