...
class Config:
    case: str = "table_1"
    # Match the full PyTorch PINN training hyperparameters for comparison.
    hidden_layers: tuple[int, ...] = (48, 48, 48)
    baseline_iterations: int = 1200
    update_iterations: int = 160
    baseline_learning_rate: float = 2.0e-3
    adaptive_learning_rate: float = 5.0e-4
    data_weight: float = 10.0
    # Forty paper observations split into five streaming update windows.
    points_per_update: int = 8
    num_domain: int = 1536
    num_boundary: int = 256
    num_initial: int = 256
    seed: int = 7
...
def make_problem(
    ...
    return dde.data.TimePDE(
        ...
        # Match torch.rand collocation sampling in the PyTorch implementation.
        train_distribution="pseudo",
    )


def make_network(cfg: Config):
    # The 2-D paper problem has three inputs; hidden depth/width match PyTorch.
    return dde.nn.FNN([3, *cfg.hidden_layers, 1], "tanh", "Glorot uniform")


def resampling_callbacks():
    """Resample interior/initial physics points every Adam iteration.

    DeepXDE requires the per-condition boundary counts to remain fixed after
    compilation, so its four separately weighted 2-D edges cannot be safely
    resampled from one shared pseudorandom boundary pool.
    """
    return [
...
def train_adaptively(cfg: Config, assimilation_points, assimilation_values):
    ...
    # PDE, four boundary losses, and IC: mirror PyTorch's 1/2/5 weighting.
    model.compile(
        "adam",
    ...
    for update, batch in enumerate(batches, start=1):
        ...
        model.compile(
            ...
            loss_weights=[1.0, 2.0, 2.0, 2.0, 2.0, 5.0, cfg.data_weight],
        )
        model.train(
